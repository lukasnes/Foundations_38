...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get('http://localhost:3000/repeat/banana')
        .then(response => {
            console.log(response.data)
            let text = document.querySelector('#repeat-text')
            text.textContent = response.data
...
// CODE HERE
const queryFunc = evt => {
    axios.get('http://localhost:3000/query-test?fruit=banana&pet=cat')
        .then(response => {
            console.log(response.data)
        })
        .catch(err => console.log(err))
